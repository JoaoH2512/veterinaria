<?php
session_start(); // sempre na primeira linha
                ...
                <div class="col-12">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover"
                            ...
                            <tbody>
                                <?php
                                // LISTA TODAS AS CONSULTAS DO CLIENTE
                                $sqlConsultas = "
                                    SELECT 
                                        c.ID_consulta,
                                        c.Data_consulta,
                                ...
                                if (empty($consultas)) {
                                    ...
                                    foreach ($consultas as $row) {
                                        ...
                                        echo "<td>" . htmlspecialchars($row['Procedimento'] ?? 'Consulta Veterinária') . "</td>";
                                        echo "<td>" . htmlspecialchars($row['nome_pet']) . "</td>";

                                        // STATUS REAL
                                        if (!empty($row['motivo_cancelamento'])) {
                                            $status = "Cancelada pelo cliente";
                                        } else {
                                            // se não tiver status no banco, considera agendada
                        ...
                        <div class="mb-4">
                            <div class="mb-3">
                                <select class="form-control" id="consultaSelecionada" required>
                                    ...
                                    <?php
                                    // CONSULTAS QUE PODEM SER REMARCADAS / CANCELADAS
                                    $sqlConsultasModal = "
                                        SELECT 
                                            c.ID_consulta,
                                            c.Data_consulta,
